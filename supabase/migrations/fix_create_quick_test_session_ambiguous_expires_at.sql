...
begin
  ...
  delete from public.device_test_sessions s
  where s.created_by = v_user_id
    and s.device_info->>'source' = 'quick_test'
    and s.expires_at <= now();

  delete from public.device_test_sessions s
  where s.id in (
    select s2.id
    from public.device_test_sessions s2
    where s2.created_by = v_user_id
      and s2.device_info->>'source' = 'quick_test'
