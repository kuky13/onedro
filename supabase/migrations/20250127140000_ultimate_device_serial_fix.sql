-- ULTIMATE FIX: Replace get_service_order_by_share_token function
-- This migration ensures the function uses imei_serial and includes all necessary fields

-- Drop all existing versions of the function
DROP FUNCTION IF EXISTS public.get_service_order_by_share_token(text);
DROP FUNCTION IF EXISTS public.get_service_order_by_share_token(uuid);

-- Create the definitive version with all required fields
...
BEGIN
  ...
  -- Return complete service order data with proper formatting
  RETURN QUERY
  SELECT 
    so.id,
    CASE 
    ...
    so.device_type,
    so.device_model,
    so.imei_serial,  -- Using imei_serial instead of device_serial
    so.reported_issue,
    so.status::varchar,
    so.priority::varchar,
    so.total_price,
...
COMMENT ON FUNCTION public.get_service_order_by_share_token(text) IS 'Returns complete service order data by share token - ULTIMATE VERSION using imei_serial'