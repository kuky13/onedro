-- Corrigir erro de tipo na função get_service_order_by_share_token
-- O campo share_token é character varying, não uuid

-- Drop da função existente
DROP FUNCTION IF EXISTS public.get_service_order_by_share_token(text);

-- Criar função corrigida sem conversão de tipo incorreta
...
BEGIN
  ...
  -- Removido ::uuid pois share_token é character varying
  IF NOT EXISTS (
...
COMMENT ON FUNCTION public.get_service_order_by_share_token(text) IS 'Returns complete service order data by share token including device password fields - Fixed type error';