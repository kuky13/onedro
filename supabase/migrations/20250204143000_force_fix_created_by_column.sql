-- Force fix admin_get_all_users_detailed function - replace created_by with owner_id
-- This migration fixes the "column created_by does not exist" error

-- Drop all existing versions of the function with all possible signatures
...
BEGIN
    ...
    LEFT JOIN (
        SELECT 
            owner_id,  -- Fixed: using owner_id instead of created_by
            COUNT(*) as count 
        FROM budgets 
        WHERE deleted_at IS NULL
        GROUP BY owner_id  -- Fixed: using owner_id instead of created_by
...
-- Add comment
COMMENT ON FUNCTION admin_get_all_users_detailed(text, text, integer, integer) IS 'Get all users with detailed information for admin panel - fixed created_by to owner_id references';