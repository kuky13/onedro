-- Ajustar duração da licença de teste para 7 dias e alinhar retorno com o frontend

-- 1) Atualizar create_trial_license para 7 dias e incluir campo license_code no JSON
...
BEGIN
  ...
  -- Gerar código de teste único (prefixo TRIAL)
  ...
  -- Calcular expiração (7 dias a partir da ativação)
...
-- 2) Atualizar activate_license_by_code para que TRIAL gere licença de 7 dias
...
BEGIN
    ...
    IF v_code LIKE 'TRIAL%' THEN
        ...
        -- Definir expiração em 7 dias a partir da ativação
        v_new_expires_at := NOW() + INTERVAL '7 days';

        -- Criar nova licença definitiva para o usuário (ainda marcada como período de teste via lógica da RPC)
        ...
        -- Registrar no histórico com ação clara de origem TRIAL
        ...
        SELECT JSON_BUILD_OBJECT(
            ...
            'code', v_new_code,
            'license_code', v_new_code,
            'expires_at', v_new_expires_at,
            'activated_at', NOW(),
            'message', 'Licença de teste de 7 dias ativada a partir de código TRIAL'
        ) INTO v_result;
