...
AS $$
BEGIN
  -- Verificar se o usuário tem acesso às ordens de serviço
  IF NOT EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE id = auth.uid() 
    AND (service_orders_beta_enabled = true OR role = 'admin')
  ) THEN
    RAISE EXCEPTION 'Acesso negado: usuário não tem permissão para acessar ordens de serviço';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Acesso negado: usuário não tem permissão para criar ordens de serviço';
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Acesso negado: usuário não tem permissão para atualizar ordens de serviço';
  END IF;
