-- Fix admin_logs column mismatch in user deletion function
-- The function was using 'admin_id' but the table uses 'admin_user_id'

-- Update the enhanced user deletion function to use correct column name
...
BEGIN
    ...
    -- Log the successful deletion (SINGLE LOG ENTRY) - FIXED: using admin_user_id instead of admin_id
...
EXCEPTION WHEN OTHERS THEN
    -- Log the error (SINGLE ERROR LOG ENTRY) - FIXED: using admin_user_id instead of admin_id
    INSERT INTO admin_logs (admin_user_id, action, details, created_at)
    VALUES (
...
'Exclui usuário completamente do sistema com logging único. Versão corrigida que usa admin_user_id.';

-- Update the compatibility function as well
...
'Função de compatibilidade que redireciona para admin_delete_user_completely_enhanced (versão corrigida).';