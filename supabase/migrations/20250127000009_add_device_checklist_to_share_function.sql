-- Atualizar função get_service_order_by_share_token para incluir device_checklist

-- Drop da função existente
DROP FUNCTION IF EXISTS public.get_service_order_by_share_token(text);

-- Criar função atualizada com device_checklist
...
BEGIN
  ...
  -- Return complete service order data with device password fields and device_checklist
  RETURN QUERY
  SELECT 
    so.id,
    CASE 
...
COMMENT ON FUNCTION public.get_service_order_by_share_token(text) IS 'Returns complete service order data by share token including device password fields and device_checklist';