...
export const EnhancedUserManagement = () => {
  ...
  const stats = useMemo(() => {
    ...
    const active = baseFilteredUsers?.filter((u: any) => u.license_active).length || 0;
    const admins = baseFilteredUsers?.filter((u: any) => u.role === 'admin').length || 0;
    const managers = baseFilteredUsers?.filter((u: any) => u.role === 'manager').length || 0;
    const totalBudgets = baseFilteredUsers?.reduce((sum: any, u: any) => sum + u.budget_count, 0) || 0;
  ...
  */
  ...
  if (!debugInfo || !(debugInfo as any).is_admin) {
    return (
      <Card>
