...
export const NewBudgetForm = ({
  ...
  const [formData, setFormData] = useState<BudgetFormData>(() => {
    ...
    if (copiedData) {
      ...
      try {
        return JSON.parse(copiedData);
      } catch (error) {
        console.error('Erro ao carregar dados copiados:', error);
      }
    }
    
    // Retornar dados padrão ou iniciais
    return initialData || {
      deviceType: 'Celular',
      deviceModel: '',
      issue: '',
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        validityDays: initialData.validity_days?.toString() || '15',
  ...
  // Handlers para busca de clientes
  const handleToggleClientSearch = () => {
  ...
  const createBudgetMutation = useMutation({
    mutationFn: async (data: BudgetFormData) => {
      ...
      const validityDays = parseInt(data.validityDays?.toString() || '15') || 15;
      const cashPriceValue = parseFloat(data.cashPrice?.toString() || '0') || 0;
      const installmentPriceValue = parseFloat(data.installmentPrice?.toString() || '0') || 0;
      
      console.log('Parsed values:', { validityDays, cashPriceValue, installmentPriceValue });

      // Validar dados obrigatórios
      if (!data.deviceType || !data.deviceModel || !data.partType) {
        throw new Error('Campos obrigatórios não preenchidos: deviceType, deviceModel ou partType');
      }

      // Calcular data de validade baseada nos dias especificados
      const validUntil = new Date();
      validUntil.setDate(validUntil.getDate() + validityDays);
      console.log('Valid until date:', validUntil.toISOString());

      const totalPrice = (data.enableInstallmentPrice && installmentPriceValue > 0)
      ...
      const { data: budget, error: budgetError } = await supabase
        ...
        .insert({
          ...
          part_quality: data.brand || null,
          part_type: data.partType,
          warranty_months: Number(data.warrantyMonths) || 0,
          cash_price: Math.round(cashPriceValue * 100),
          installment_price: (data.enableInstallmentPrice && installmentPriceValue > 0) ? Math.round(installmentPriceValue * 100) : null,
          installments: data.enableInstallmentPrice ? Number(data.installments) || 1 : 1,
          ...
          payment_condition: data.paymentCondition || null,
      ...
      if (budgetError) {
        console.error('Budget creation error:', budgetError, {
          code: (budgetError as any)?.code,
          detail: (budgetError as any)?.details,
          hint: (budgetError as any)?.hint
        });
        throw budgetError;
      }
      ...
      const { error: partError } = await supabase
        ...
        .insert({
          ...
          cash_price: Math.round(cashPriceValue * 100),
          installment_price: (data.enableInstallmentPrice && installmentPriceValue > 0) ? Math.round(installmentPriceValue * 100) : null,
          installment_count: data.enableInstallmentPrice ? Number(data.installments) || 0 : 0,
          warranty_months: Number(data.warrantyMonths) || 0,
        });
      ...
      if (partError) {
        console.error('Budget part creation error:', partError, {
          ...
          detail: (partError as any)?.details,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Form submit started:', { user, formData });
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label htmlFor="issue">Defeito do Dispositivo</Label>
        ...
        {/* Client Section - usando o mesmo componente do formulário de edição */}
        <ClientSection
          formData={{
          ...
          onToggleClientSearch={handleToggleClientSearch}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label htmlFor="paymentCondition">Método de Pagamento</Label>
  ...
  // Load default validity from site_settings
  const { data: siteSettings } = useQuery({
    queryKey: ['site-settings-default-validity'],
    queryFn: async () => {
      const { data, error } = await supabase.from('site_settings').select('default_budget_validity_days').maybeSingle();
      if (error) {
        console.error('Erro ao buscar default de validade:', error);
        return null;
      }
      return data;
    }
  ...
  useEffect(() => {
    const defaultDays = siteSettings?.default_budget_validity_days;
    if (!initialData && defaultDays && typeof defaultDays === 'number') {
      setFormData(prev => ({ ...prev, validityDays: String(defaultDays) }));
    }
  }, [siteSettings, initialData]);
