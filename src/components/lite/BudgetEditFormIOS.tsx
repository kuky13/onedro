...
export const BudgetEditFormIOS = ({
  ...
  const [formData, setFormData] = useState({
    ...
    // Nova lógica: controlar dias de validade e calcular a data
    validity_days: (() => {
      try {
        const created = new Date(budget.created_at);
        const validUntil = budget.expires_at ? new Date(budget.expires_at) : null;
        if (created instanceof Date && !isNaN(created.getTime()) && validUntil instanceof Date && !isNaN(validUntil.getTime())) {
          const diffMs = validUntil.getTime() - created.getTime();
    ...
    expires_at: budget.expires_at ? new Date(budget.expires_at).toISOString().split('T')[0] : '',
  });
        ...
        setFormData({
          ...
          validity_days: (() => {
            try {
              ...
              const validUntil = data.expires_at ? new Date(data.expires_at) : null;
              if (created instanceof Date && !isNaN(created.getTime()) && validUntil instanceof Date && !isNaN(validUntil.getTime())) {
                const diffMs = validUntil.getTime() - created.getTime();
                const days = Math.max(1, Math.round(diffMs / (24 * 60 * 60 * 1000)));
                return days;
  ...
  const handleInputChange = (field: string, value: string | number | boolean) => {
    // Atualizar campos com lógica de sincronização entre validade em dias e data
    if (field === 'validity_days') {
      const daysNum = Math.max(1, Math.min(365, Number(value) || 15));
      const createdBase = budget.created_at ? new Date(budget.created_at) : new Date();
      createdBase.setHours(0, 0, 0, 0);
      const validUntilDate = new Date(createdBase.getTime() + daysNum * 24 * 60 * 60 * 1000);
    ...
    if (field === 'expires_at' && typeof value === 'string' && value) {
      // Quando a data muda, recalcular dias de validade com base em created_at
      const createdBase = budget.created_at ? new Date(budget.created_at) : new Date();
      createdBase.setHours(0, 0, 0, 0);
      const newDate = new Date(value);
      if (!isNaN(newDate.getTime())) {
        const diffMs = newDate.getTime() - createdBase.getTime();
        const days = Math.max(1, Math.round(diffMs / (24 * 60 * 60 * 1000)));
        setFormData(prev => ({
          ...prev,
          expires_at: value,
  ...
  const handleSave = async () => {
    setIsLoading(true);
    try {
      // Calcular a data de expiração baseada nos dias de validade
      const createdBase = budget.created_at ? new Date(budget.created_at) : new Date();
      createdBase.setHours(0, 0, 0, 0);
      const days = Number(formData.validity_days) || 15;
      const validUntilDate = new Date(createdBase.getTime() + Math.max(1, Math.min(365, days)) * 24 * 60 * 60 * 1000);
      const validUntilISO = validUntilDate.toISOString().split('T')[0];
          ...
          {isLoadingData ? (
            ...
            <>
              ...
              <PricingSection
                formData={{
                  ...formData,
                  total_price: formData.cash_price || 0
                }}
                onInputChange={handleInputChange}
              />
