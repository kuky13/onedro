...
export const ClientsLite = ({
    ...
    try {
      ...
      const clientsWithCount = await Promise.all((clientsData || []).map(async client => {
        try {
          const {
            data: budgetCount,
            error: budgetCountError
          } = await supabase.rpc('get_client_budget_count', {
            client_id: client.id
          });
          const {
            data: serviceOrderCount,
            error: serviceOrderCountError
          } = await supabase.rpc('get_client_service_order_count', {
            client_id: client.id
          });
          console.log('[ClientsLite] RPC counts', {
            clientId: client.id,
            budgetCount,
            serviceOrderCount,
            budgetCountError,
            serviceOrderCountError
          });
          if (budgetCountError) {
            console.error('Error counting budgets for client:', client.id, budgetCountError);
          }
          if (serviceOrderCountError) {
            console.error('Error counting service orders for client:', client.id, serviceOrderCountError);
          }
          // Fallback: contagem direta via SELECT quando RPC falhar ou retornar zero
          let finalBudgetCount = typeof budgetCount === 'number' ? budgetCount : 0;
          if (budgetCountError || finalBudgetCount === 0) {
            if (!userId || !client?.id) {
              console.warn('[ClientsLite] Skipping budgets direct count: missing userId or clientId', {
                userId,
                clientId: client?.id
              });
            } else {
              try {
                const {
                  count: budgetDirectCount,
                  error: budgetDirectError
                } = await supabase.from('budgets').select('id', {
                  count: 'exact'
                }).eq('client_id', client.id).eq('owner_id', userId).is('deleted_at', null);
                if (budgetDirectError) {
                  console.error('Direct count error for budgets:', {
                    clientId: client.id,
                    userId,
                    error: budgetDirectError
                  });
                } else if (typeof budgetDirectCount === 'number') {
                  console.log('[ClientsLite] Fallback budgets count', {
                    clientId: client.id,
                    userId,
                    budgetDirectCount
                  });
                  finalBudgetCount = budgetDirectCount;
                }
              } catch (e: any) {
                if (e?.name === 'AbortError') {
                  console.warn('[ClientsLite] Budgets count aborted, retry skipped', {
                    clientId: client.id,
                    userId
                  });
                } else {
                  console.error('[ClientsLite] Unexpected error counting budgets directly', {
                    clientId: client.id,
                    userId,
                    error: e
                  });
          ...
          let finalServiceOrderCount = typeof serviceOrderCount === 'number' ? serviceOrderCount : 0;
          if (serviceOrderCountError || finalServiceOrderCount === 0) {
            if (!userId || !client?.id) {
              console.warn('[ClientsLite] Skipping service orders direct count: missing userId or clientId', {
                userId,
                clientId: client?.id
              });
            } else {
              try {
                const {
                  count: serviceOrderDirectCount,
                  error: serviceOrderDirectError
                } = await supabase.from('service_orders').select('id', {
                  count: 'exact'
                }).eq('client_id', client.id).eq('owner_id', userId).is('deleted_at', null);
                if (serviceOrderDirectError) {
                  console.error('Direct count error for service orders:', {
                    clientId: client.id,
                    userId,
                    error: serviceOrderDirectError
                  });
                } else if (typeof serviceOrderDirectCount === 'number') {
                  console.log('[ClientsLite] Fallback service orders count', {
                    clientId: client.id,
                    userId,
                    serviceOrderDirectCount
                  });
                  finalServiceOrderCount = serviceOrderDirectCount;
                }
              } catch (e: any) {
                if (e?.name === 'AbortError') {
                  console.warn('[ClientsLite] Service orders count aborted, retry skipped', {
                    clientId: client.id,
                    userId
                  });
                } else {
                  console.error('[ClientsLite] Unexpected error counting service orders directly', {
                    clientId: client.id,
                    userId,
                    error: e
                  });
          ...
          console.log('[ClientsLite] Final counts', {
            clientId: client.id,
            budget: finalBudgetCount,
            serviceOrders: finalServiceOrderCount
          });
          return {
            ...client,
            budget_count: finalBudgetCount,
            service_order_count: finalServiceOrderCount
          };
        } catch (error) {
          console.error('Error calling get_client_budget_count for client:', client.id, error);
          return {
            ...client,
            budget_count: 0,
      ...
      setClients(clientsWithCount as Client[]);
    } catch (error) {
  ...
  const deleteClient = async (clientId: string) => {
    try {
      ...
      toast({
        ...
        description: error.message.includes('Cliente padrão') ? 'O cliente padrão não pode ser excluído.' : 'Não foi possível excluir o cliente. Tente novamente.',
        variant: 'destructive'
      });
    } finally {
      setIsDeleting(null);
  ...
  const filteredClients = clients.filter(client => client.name.toLowerCase().includes(searchTerm.toLowerCase()) || client.phone.includes(searchTerm) || client.email?.toLowerCase().includes(searchTerm.toLowerCase()) || client.address?.toLowerCase().includes(searchTerm.toLowerCase()) || client.city?.toLowerCase().includes(searchTerm.toLowerCase()) || client.notes?.toLowerCase().includes(searchTerm.toLowerCase()));
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="col-span-2">
                ...
                <Input id="zip_code" value={formData.zip_code} onChange={e => setFormData({
                ...formData,
              ...
              })} placeholder="Observações sobre o cliente..." rows={3} />
              </div>
              <div className="col-span-2">
                <div className="flex items-center space-x-2">
                  <input type="checkbox" id="is_favorite" checked={formData.is_favorite} onChange={e => setFormData({
                  ...formData,
