...
export const BudgetFormLite = ({
  ...
  // Buscar padrão de dias de validade em site_settings e aplicar quando criando
  useEffect(() => {
    const loadDefaultValidity = async () => {
      try {
        ...
        if (!error && data?.default_budget_validity_days && !initialData) {
          setFormData(prev => ({ ...prev, validityDays: String(data.default_budget_validity_days) }));
        }
      } catch (err) {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        const {
          ...
          installment_count: formData.enableInstallmentPrice ? formData.installments : 0,
          warranty_months: formData.warrantyMonths
        });
