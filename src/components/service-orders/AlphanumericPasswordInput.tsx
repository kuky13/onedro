...
export const AlphanumericPasswordInput: React.FC<AlphanumericPasswordInputProps> = ({
  ...
  const validatePassword = (password: string): {
    ...
    if (!passwordRegex.test(password)) {
      ...
      if (!/^[a-zA-Z0-9]+$/.test(password)) {
        return {
          isValid: false,
          error: 'Senha deve conter apenas letras e números'
        };
      }
    }
    return {
  ...
  const getStrengthIndicator = () => {
    ...
    return <p className={`text-xs ${strengthColor}`}>
        Força da senha: {strengthText}
      </p>;
  };
  return <div className="space-y-2">
      <div className="relative">
        <Input type={showPassword ? "text" : "password"} value={value} onChange={handleChange} placeholder="senha123" disabled={disabled} className={getInputClassName()} maxLength={20} />
        
        <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
          {value && isValid !== null && <div>
              {isValid ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-red-500" />}
            </div>}
          
          <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setShowPassword(!showPassword)} disabled={disabled}>
      ...
      {value && isValid && <div className="space-y-1">
