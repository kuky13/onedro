...
export const PWAInstallModalSimple: React.FC<PWAInstallModalSimpleProps> = ({
  ...
  const getPlatformBadge = () => {
    switch (platform) {
      case 'android':
        return (
          <Badge variant="secondary" className="bg-green-100 text-green-800 border-green-200 flex items-center gap-1 px-2 py-1 text-xs">
      ...
      case 'ios':
        return (
          <Badge variant="secondary" className="bg-blue-100 text-blue-800 border-blue-200 flex items-center gap-1 px-2 py-1 text-xs">
      ...
      case 'desktop':
        return (
          <Badge variant="secondary" className="bg-purple-100 text-purple-800 border-purple-200 flex items-center gap-1 px-2 py-1 text-xs">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xs sm:max-w-sm md:max-w-md w-full mx-2 sm:mx-4 p-0 overflow-hidden rounded-xl sm:rounded-2xl border-0 shadow-lg sm:shadow-2xl max-h-[90vh] overflow-y-auto bg-background">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 bg-background">
          {/* Status card moderno */}
          <div className={cn(
            "relative flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg sm:rounded-xl border-2 transition-colors duration-200",
            "shadow-sm",
            isInstalled 
              ? "bg-background border-green-500/30" 
              : "bg-gradient-to-r from-[#fec832]/10 to-[#f5b800]/10 border-[#fec832]/30"
          )}>
            <div className={cn(
              "flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center",
              isInstalled 
                ? "bg-green-500/20 text-green-400" 
                : "bg-[#fec832]/20 text-[#e6a600]"
            )}>
              {isInstalled ? (
            ...
            <div className={cn(
              ...
              isInstalled ? "bg-green-400" : "bg-[#fec832]"
            )} />
          </div>
          ...
          {!isInstalled && (
            <Button
              ...
              className={cn(
                "w-full h-12 sm:h-14 text-sm sm:text-base font-semibold rounded-lg sm:rounded-xl transition-colors duration-200",
                "bg-gradient-to-r from-[#fec832] to-[#f5b800] hover:from-[#e6b52e] hover:to-[#e6a600]",
                "text-black border-0 shadow-md hover:shadow-lg",
                "disabled:opacity-70"
              )}
            >
              {isInstalling ? (
          ...
          {instructions && showInstructions && (
            <div className="bg-card border border-border rounded-lg sm:rounded-xl p-4 sm:p-5 space-y-3 sm:space-y-4 shadow-sm">
          ...
          {instructions && !showInstructions && !isInstalled && (
            <Button
              onClick={() => setShowInstructions(true)}
              variant="outline"
              className="w-full justify-between h-10 sm:h-12 rounded-lg sm:rounded-xl border-2 hover:bg-muted transition-colors duration-200"
            >
          ...
          <div className="bg-card border border-border rounded-lg sm:rounded-xl p-4 sm:p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-card/50 rounded-lg border border-border/30">
                ...
                <span className="text-xs sm:text-sm font-medium text-foreground">Acesso offline aos seus dados</span>
              </div>
          ...
          {isInstalled && (
            <Button
              ...
              variant="outline"
              className="w-full h-10 sm:h-12 rounded-lg sm:rounded-xl border-2 font-medium hover:bg-muted transition-colors duration-200"
            >
              Fechar
            </Button>
          )}
