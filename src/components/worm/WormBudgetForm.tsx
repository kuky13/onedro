...
import { Loader2, Phone, Smartphone, DollarSign, CheckCircle, FileText, X } from 'lucide-react';
...
export const WormBudgetForm = ({ budget, initialData, onSuccess, onCancel }: WormBudgetFormProps) => {
  ...
  const {
    ...
    defaultValues: budget ? {
      ...
      total_price: budget.total_price ? budget.total_price / 100 : 0,
      cash_price: budget.cash_price ? budget.cash_price / 100 : undefined,
      ...
      total_price: initialData.total_price ? initialData.total_price / 100 : 0,
      cash_price: initialData.cash_price ? initialData.cash_price / 100 : undefined,
      installment_price: initialData.installment_price ? initialData.installment_price / 100 : undefined,
  ...
  const onSubmit = async (data: BudgetFormData) => {
    ...
    try {
      ...
      const calculatedTotalPrice = totalPriceValue > 0
        ? Math.round(totalPriceValue * 100)
        : (data.enable_installment_price && installmentPriceValue > 0)
          ? Math.round(installmentPriceValue * 100)
          : Math.round(cashPriceValue * 100);
      ...
      const validityDays = Number.isFinite(data.validity_days)
        ? (data.validity_days as number)
        : Math.max(1, Math.min(365, profile?.budget_warning_days ?? 15));
      ...
      const budgetData = {
        ...
        client_name: clientName, // Usar a variável validada
        client_phone: selectedClient?.phone || data.client_phone,
        issue: data.issue || null,
        total_price: finalTotalPrice,
        cash_price: Math.round(cashPriceValue * 100),
        installment_price: data.enable_installment_price && installmentPriceValue > 0 ? Math.round(installmentPriceValue * 100) : null,
        ...
        owner_id: user.id,
        // PROTEÇÃO 3: Garantir workflow_status correto
      ...
      await queryClient.invalidateQueries({ queryKey: ['worm-budgets'] });
    ...
    <div className="flex flex-col h-full">
      <SheetHeader className="shrink-0 bg-background border-b border-border/20 p-4 sm:p-6">
        <div className="space-y-2">
          <SheetTitle className="text-lg sm:text-xl font-semibold text-center sm:text-left">
            {isEditing ? 'Editar Orçamento' : 'Novo Orçamento'}
          </SheetTitle>
          {/* Exibição do número OS quando editando */}
          {isEditing && budget && (
            <div className="flex justify-center sm:justify-start">
              <span className="bg-primary/10 px-3 py-1.5 rounded-md text-sm font-mono font-medium text-primary">
                {budget.sequential_number 
              ? `OS: ${budget.sequential_number.toString().padStart(4, '0')}` 
              : budget.id ? `OS-${budget.id.slice(-8)}` : 'OS-NOVO'
            }
              </span>
            </div>
          )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full">
        <div 
          ...
          className="flex-1 overflow-y-auto scroll-smooth form-scroll-container space-y-3 sm:space-y-4 p-4 sm:p-6 pb-4"
        >
            {/* Cliente */}
              ...
              <CardContent className="space-y-3 px-3 sm:px-6">
                ...
                {!selectedClientId && (
                  <div className="space-y-3 animate-in slide-in-from-top-2 duration-300">
                    <div>
                      <Label htmlFor="client_name" className="text-xs sm:text-sm font-medium">Nome do Cliente *</Label>
                      <Input
                        id="client_name"
                    ...
                    <div>
                      <Label htmlFor="client_phone" className="text-xs sm:text-sm font-medium">Telefone</Label>
                      <Input
                        id="client_phone"
                ...
                {selectedClientId && selectedClient && (
                  <div className="p-3 bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg border border-primary/20 animate-in slide-in-from-top-2 duration-300">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {selectedClient.phone && (
                          <div className="text-xs text-muted-foreground mt-1">{selectedClient.phone}</div>
                        )}
            ...
            <Card className="border-border/40 shadow-sm">
              ...
              <CardContent className="space-y-3 px-3 sm:px-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="device_type" className="text-xs sm:text-sm font-medium">Tipo de Dispositivo *</Label>
                  ...
                  <div>
                    <Label htmlFor="device_model" className="text-xs sm:text-sm font-medium">Modelo *</Label>
                    <Input
                      id="device_model"
                  ...
                  <div>
                    <Label htmlFor="part_quality" className="text-xs sm:text-sm font-medium">Nome do Reparo</Label>
                    <Input
                      id="part_quality"
                      {...register('part_quality')}
                      placeholder="Ex: Troca de Tela, Original, Premium..."
                      className="mt-1 h-9 sm:h-10 text-sm"
                    />
                  ...
                  {/* Dias de Validade */}
                  <div>
                    <Label htmlFor="validity_days" className="text-xs sm:text-sm font-medium">Dias de Validade</Label>
                    <Input
                      id="validity_days"
                      type="number"
                      min={1}
                    ...
                    <div className="flex justify-between items-center mt-1">
                      ...
                      {errors.validity_days && (
                        <p className="text-[11px] sm:text-xs text-destructive">{errors.validity_days.message}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <CardContent className="space-y-3 px-3 sm:px-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="includes_delivery" className="text-xs sm:text-sm">
                      🚚 Entrega incluída
                    </Label>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="includes_screen_protector" className="text-xs sm:text-sm">
                      🛡️ Película protetora
                    </Label>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Ex: Limpeza interna, troca de capa traseira, backup de dados..."
                    rows={3}
                    maxLength={30}
                  ...
                  <div className="flex justify-between items-center mt-1">
                    ...
                    <div className={`text-xs ${
                      (watch('custom_services')?.length || 0) > 24 
                        ? 'text-orange-500' 
                        : (watch('custom_services')?.length || 0) > 27 
                          ? 'text-red-500' 
            ...
            <Card className="border-border/40 shadow-sm">
              ...
              <CardContent className="px-3 sm:px-6">
                <div>
                  <Label htmlFor="notes" className="text-xs sm:text-sm font-medium">Observações Gerais</Label>
                  <Textarea
                    id="notes"
                    {...register('notes')}
                  ...
                  <div className="flex justify-between items-center mt-1">
                    ...
                    <div className={`text-xs ${
                      (watch('notes')?.length || 0) > 60 
                        ? 'text-orange-500' 
                        : (watch('notes')?.length || 0) > 70 
                          ? 'text-red-500' 
                          : 'text-muted-foreground'
                    }`}>
        ...
        <div className="shrink-0 bg-background/95 backdrop-blur-sm border-t border-border/30 p-3 sm:p-4 md:p-6 shadow-lg sticky bottom-0 z-10">
          <div className="max-w-md mx-auto">
            <div className="flex flex-col xs:flex-row gap-2 sm:gap-3">
              <Button 
                ...
                variant="outline" 
                onClick={onCancel}
                className="flex-1 xs:flex-none xs:min-w-[120px] h-10 sm:h-11 text-sm sm:text-base font-medium border-2 hover:bg-muted/50 transition-all duration-200 active:scale-[0.98]"
              >
              ...
              <Button 
                type="submit" 
                disabled={isLoading}
                className="flex-1 xs:flex-none xs:min-w-[160px] h-10 sm:h-11 text-sm sm:text-base font-medium bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow-md hover:shadow-lg transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden xs:inline">{isEditing ? 'Atualizar Orçamento' : 'Criar Orçamento'}</span>
                    <span className="xs:hidden">{isEditing ? 'Atualizar' : 'Criar'}</span>
                  </div>
                )}
              </Button>
            </div>
