...
interface BudgetViewModalProps {
  ...
  onPDF?: (budget: Budget) => void;
  onWhatsApp?: (budget: Budget) => void;
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-lg">{budget.cash_price ? formatCurrencyFromReais(budget.cash_price) : 'Não informado'}</p>
                </div>
                {budget.installment_price && <div>
                    <p className="text-sm text-muted-foreground">Valor Parcelado</p>
                    <p className="font-semibold text-lg">{budget.installment_price ? formatCurrencyFromReais(budget.installment_price) : 'Não informado'}</p>
                  </div>}
              </div>
