...
export const useSecureAuth = () => {
  ...
  const signUp = async ({ email, password, name }: SignUpCredentials) => {
    try {
      ...
      showSuccess({
        ...
        description: 'Verifique seu email para confirmar a conta. Se não encontrar o email na caixa de entrada, verifique também a pasta de spam/lixo eletrônico.',
        duration: 8000
      });
  ...
  const resetPassword = async (email: string) => {
    try {
      ...
      showSuccess({
        title: 'Email enviado',
        description: 'Verifique seu email para redefinir a senha. Se não encontrar o email na caixa de entrada, verifique também a pasta de spam/lixo eletrônico.',
        duration: 8000
      });
