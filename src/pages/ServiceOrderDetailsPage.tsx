...
export const ServiceOrderDetailsPage = () => {
  ...
  const { actions, executeAction, canExecuteAction, getStatusColor, getStatusText, getStatusIcon } = useContextualActions(serviceOrder);
  ...
  const getPriorityColor = (priority: string) => {
    const colors = {
      ...
      medium: 'bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-200',
        ...
        <Card className="border-border/50">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(serviceOrder.status)}`}>
                  {getStatusIcon(serviceOrder.status)}
                  <span className="ml-1">{getStatusText(serviceOrder.status)}</span>
                </span>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-sm ml-1">R$ {(serviceOrder.labor_cost || 0).toFixed(2)}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <DollarSign className="h-4 w-4 text-muted-foreground" />
                <div>
                  ...
                  <span className="text-sm ml-1">R$ {(serviceOrder.parts_cost || 0).toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-sm ml-1 font-bold">R$ {(serviceOrder.total_price || 0).toFixed(2)}</span>
              </div>
            </div>
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="font-medium">R$ {(item.unit_price || 0).toFixed(2)}</div>
